...
\pi(\alpha \mid \ldots) \propto \prod_{i=1}^{n}\left[\tau^{\frac{1}{2}} e^{-\tau\frac{\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}}{2}}\right]\\

\pi(\alpha \mid \ldots) \propto  e^{-\frac{\tau\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}\right]}{2}}
$$
...
\\
\pi(\beta \mid \ldots) \propto \prod_{i=1}^{n}\left[\tau^{\frac{1}{2}}e^{-\tau\frac{\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}}{2}}\right]
\\
\pi(\beta \mid \ldots) \propto  e^{-\frac{\tau\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}\right]}{2}}
$$
...
\\
\pi(\gamma \mid \ldots) \propto \prod_{i=1}^{n}\left[\tau^{\frac{1}{2}}e^{-\tau\frac{\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}}{2}}\right]
\\
\pi(\gamma \mid \ldots) \propto  e^{-\frac{\tau\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}\right]}{2}}
$$
...
\\
\pi(\tau \mid \ldots) \propto  \tau^{\frac{n}{2}}e^{-\frac{\tau\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}\right]}{2}}
$$
...
dugong <- function(age, length, nchain = 10^4, prop_sd = c(2,6,2,2)){
  ...
  valeur_extreme_params =cbind(c(1+10^(-7),1+10^(-7), 10^(-7),10^(-7)),c(10^7,10^7,10^7,1-10^(-7) ))
  ...
  for (iter in 1:nchain){
    ...
    for (j in 1:4){
      ...
      top <-   sum( 0.5*log(prop[3]) - 0.5*(length - prop[1] - prop[2]*prop[4]^age)^2 * prop[3] ) + log(top_kernel)
      bottom <-  sum( 0.5*log(current[3]) - 0.5*(length -current[1] - current[2]*current[4]^age)^2 *current[3]) + log(bottom_kernel)
...
#on trace les densité 
par(mfrow = c(1, 2), mar = c(4, 5, 0.5, 0.5))
print(density(CM_params$chain[,2]))
