...
\\
\pi(\alpha \mid \ldots) \propto \prod_{i=1}^{n}\left[\frac{1}{\sqrt{\tau}} e^{-\frac{\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}}{2\tau}}\right]
\\
\pi(\alpha \mid \ldots) \propto  e^{-\frac{\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}\right]}{2\tau}}
...
\\
\pi(\beta \mid \ldots) \propto \prod_{i=1}^{n}\left[\frac{1}{\sqrt{\tau}} e^{-\frac{\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}}{2\tau}}\right]
\\
\pi(\beta \mid \ldots) \propto  e^{-\frac{\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}\right]}{2\tau}}
...
\\
\pi(\gamma \mid \ldots) \propto \prod_{i=1}^{n}\left[\frac{1}{\sqrt{\tau}} e^{-\frac{\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}}{2\tau}}\right]
\\
\pi(\gamma \mid \ldots) \propto  e^{-\frac{\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}\right]}{2\tau}}



$$
$$
\pi(\tau \mid \ldots) \propto \prod_{i=1}^{n} \pi\left(y_{i} \mid \alpha, \beta, \gamma, \tau\right) \times \pi(\gamma)
\\
\pi(\tau \mid \ldots) \propto \prod_{i=1}^{n}\left[\frac{1}{\sqrt{\tau}} e^{-\frac{\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}}{2\tau}}\right]
\\
\pi(\tau \mid \ldots) \propto  \tau^{-\frac{n}{2}}e^{-\frac{\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha-\beta\gamma^{x_{i}}\right)^{2}\right]}{2\tau}}
...
dugong <- function(age, length, nchain = 10^4, prop_sd = c(0.5,1,2,0.2)){

  init <- c(3, 2, 1, 0.1)  #alpha, beta (alpha et beta superieur à 1), tau (standard deviation donc on commence à 1) et gamma (entre 0 et 1 donc on prend 0.5)   
  chain <- matrix(NA, nchain + 1, 4)
  chain[1,] <- init
  acc_rates <- rep(0, 4)
  
  valeur_extreme_params =cbind(c(1,1, 0.01,0),c(10^5,10^5,10^5,1 ))
  ...
  for (iter in 1:nchain){
    ...
    for (j in 1:4){
      ...
      prop <- current
      prop[j] <- rtruncnorm(1, a=valeur_extreme_params[j,1] , b=valeur_extreme_params[j,2] , current[j], prop_sd[j])
      bottom_kernel <- dtruncnorm(prop[j], a=valeur_extreme_params[j,1] , b=valeur_extreme_params[j,2] , current[j], prop_sd[j])  #on a pas un noyau symetrique !
      top_kernel <- dtruncnorm(current[j], a=valeur_extreme_params[j,1] , b=valeur_extreme_params[j,2] , prop[j], prop_sd[j]) #on a pas un noyau symetrique !
      
      
      top <-   sum( 0.5*log(prop[3]) - 0.5*(length - prop[1] - prop[2]*prop[4]^age)^2 * prop[3] ) + top_kernel
      bottom <-  sum( 0.5*log(current[3]) - 0.5*(length -current[1] - current[2]*current[4]^age)^2 *current[3]) + bottom_kernel
...
head(CM_params$chain,5)
CM_params$acc_rates


#CM_params
## to do remove burning period


#plot
for (j in 1:4)
  plot(CM_params$chain[,j], type = "l", ylab = j)
