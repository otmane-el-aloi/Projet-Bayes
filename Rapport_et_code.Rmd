...
dugong <- function(age, Y, nchain = 10^4, prop_sd = c(0.08,0.06,0.01,0.03)){

  init <- c(2.5, 0.9, 100, 0.9)  #alpha, beta (alpha et beta superieur à 1), tau (standard deviation donc on commence à 1) et gamma (entre 0 et 1 donc on prend 0.5)   
  ...
  for (iter in 1:nchain){
    ...
    ## Mise à jour de tau   
      ...
      #Top
      top <- (n/2)*log(prop[3]) - sum(((Y - prop[1] - prop[2]*prop[4]^age)^2)*1/2                *prop[3]) +dgamma(prop[3],0.001,rate=0.001,log=TRUE)
      #Bottom
      bottom <- (n/2)*log(current[3]) - sum(((Y - current[1] - current[2]                           *current[4]^age)^2)*1/2*current[3])
                +dgamma(current[3],0.001,rate=0.001,log=TRUE)   
   ...
   ### Mise à jour de gamma
      prop <- current
      prop[4] <- rtruncnorm(1, a=0 , b=1 , current[4], prop_sd[4])
      top_kernel <- dtruncnorm(current[4], a=0 , b=1 , prop[4], prop_sd[4]) 
      bottom_kernel <- dtruncnorm(prop[4], a=0 , b=1 , current[4], prop_sd[4])    
      
      #on a pas un noyau symetrique !
      kernel.ratio<-(top_kernel/bottom_kernel)
      
      #top
      top <- (n/2)*log(prop[3]) - sum(((Y - prop[1] - prop[2]*prop[4]^age)^2)*1/2                *prop[3]) +dunif(prop[4],min=0,max=1,log=TRUE)
      
      bottom <- (n/2)*log(current[3]) - sum(((Y - current[1] - current[2]                        *current[4]^age)^2)*1/2*current[3])+
               dunif(current[4],min=0,max=1,log=TRUE)
      #Alpha
      acc_prob <- exp(top - bottom)*kernel.ratio
