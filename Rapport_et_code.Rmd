...
\\
\pi(\alpha \mid \ldots) \propto \prod_{i=1}^{n}\left[\tau^{-1/2}e^{-\frac{\left(Y_{i}-\alpha+\beta\gamma^{x_{i}}\right)^{2}}{2\tau}}\right]\times \pi(\alpha)
\\
\pi(\alpha \mid \ldots) \propto  \tau^{-n/2}\times e^{-\frac{\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha+\beta\gamma^{x_{i}}\right)^{2}\right]}{2\tau}}\times \mathbb{1_{[1,+\infty[}{\alpha}}
$$

$$
\pi(\beta \mid \ldots) \propto \prod_{i=1}^{n} \pi\left(y_{i} \mid \alpha, \beta, \gamma, \tau\right) \times \pi(\beta)
\\
\pi(\beta \mid \ldots) \propto \prod_{i=1}^{n}\left[\tau^{-1/2}e^{-\frac{\left(Y_{i}-\alpha+\beta\gamma^{x_{i}}\right)^{2}}{2\tau}}\right]\times \pi(\beta)
\\
\pi(\beta \mid \ldots) \propto \tau^{-n/2}\times e^{-\frac{\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha+\beta\gamma^{x_{i}}\right)^{2}\right]}{2\tau}}\times \mathbb{1_{[1,+\infty[}{\beta}}
$$

$$
\pi(\gamma \mid \ldots) \propto \prod_{i=1}^{n} \pi\left(y_{i} \mid \alpha, \beta, \gamma, \tau\right) \times \pi(\gamma)
\\
\pi(\gamma \mid \ldots) \propto \prod_{i=1}^{n}\left[\tau^{-1/2}e^{-\frac{\left(Y_{i}-\alpha+\beta\gamma^{x_{i}}\right)^{2}}{2\tau}}\right] \times \pi(\gamma)
\\
\pi(\gamma \mid \ldots) \propto \tau^{-n/2}\times e^{-\frac{\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha+\beta\gamma^{x_{i}}\right)^{2}\right]}{2\tau}}\times \mathbb{1_{]0,1[}{\gamma}}
$$

$$
\pi(\tau \mid \ldots) \propto \prod_{i=1}^{n} \pi\left(y_{i} \mid \alpha, \beta, \gamma, \tau\right) \times \pi(\tau)
\\
\pi(\tau \mid \ldots) \propto \prod_{i=1}^{n}\left[\tau^{\frac{-1}{2}}e^{-\tau\frac{\left(Y_{i}-\alpha+\beta\gamma^{x_{i}}\right)^{2}}{2}}\right]
\\
\pi(\tau \mid \ldots) \propto  \tau^{-n/2}\times e^{-\frac{\sum_{i=1}^{n}\left[\left(Y_{i}-\alpha+\beta\gamma^{x_{i}}\right)^{2}\right]}{2\tau}}
$$
...
dugong <- function(age, Y, nchain = 10^4, prop_sd = c(0.08,0.06,0.5,0.009)){
  ...
  for (iter in 1:nchain){
    ...
    for (j in 1:4){
      ...
      #loi normle comme loi à priori
      top <- -(n/2)*log(prop[3]) - sum((Y - prop[1] - prop[2]*prop[4]^age)^2)*(1/(2*prop[3])) +dnorm(prop[j],0,sig,log=TRUE)
      
      bottom <- -(n/2)*log(current[3]) - sum((Y - current[1] - current[2]*current[4]^age)^2)*(1/(2*current[3]))+dnorm(current[j],0,sig,log=TRUE) }
      ...
      kernel.ratio<-(prop[j]/current[j])
      #loi normle comme loi à priori
      top <- -(n/2)*log(prop[3]) - sum((Y - prop[1] - prop[2]*prop[4]^age)^2)*(1/(2*prop[3])) +dinvgamma(prop[j],0.001,rate=1000,log=TRUE)
      
      bottom <- -(n/2)*log(current[3]) - sum((Y - current[1] - current[2]*current[4]^age)^2)*(1/(2*current[3]))+dinvgamma(current[j],0.001,rate=1000,log=TRUE)     
        
      }else{
         
        prop[j] <- rtruncnorm(1, a=0 , b=1 , current[j], prop_sd[j])
        bottom_kernel <- dtruncnorm(prop[j], a=0 , b=1 , current[j], prop_sd[j])  #on a pas un noyau symetrique !
        top_kernel <- dtruncnorm(current[j], a=0 , b=1 , prop[j], prop_sd[j]) #on a pas un noyau symetrique !
         kernel.ratio<-(top_kernel/bottom_kernel)
      #loi normle comme loi à priori
      top <- -(n/2)*log(prop[3]) - sum((Y - prop[1] - prop[2]*prop[4]^age)^2)*(1/(2*prop[3])) +dunif(prop[j],min=0,max=1,log=TRUE)
      
      bottom <- -(n/2)*log(current[3]) - sum((Y - current[1] - current[2]*current[4]^age)^2)*(1/(2*current[3]))+dunif(current[j],min=0,max=1,log=TRUE)
